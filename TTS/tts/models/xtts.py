...
class Xtts(BaseTTS):
    ...
    def inference(
        self,
        text, # after modify, text is a list of sentences
        language,
        gpt_cond_latent,
        speaker_embedding,
        # GPT inference
        ...
        **hf_generate_kwargs,
        ...
        for sent in text:
            ...
            encoded_text = torch.IntTensor(self.tokenizer.encode(sent, lang=language))
            text_ids.append(encoded_text)
            max_text_len = max(max_text_len, len(encoded_text))
        ...
        for t in text_ids:
            text_ids_for_pad.append(torch.cat([t, torch.full((max_text_len - t.size(0),), pad_token_id)]))
            text_pad_lengths.append(max_text_len - t.size(0))

        text_inputs = torch.stack(text_ids_for_pad).to(self.device)
        assert (
            text_inputs.shape[-1] < self.args.gpt_max_text_tokens
        ), " ❗ XTTS can only generate text with a maximum of 400 tokens."

        batch_cond_latent = gpt_cond_latent.repeat(len(text_ids), 1, 1)
        with torch.no_grad():
            batch_gpt_codes = self.gpt.generate(
                cond_latents=batch_cond_latent,
                text_inputs=text_inputs,
                text_pad_lengths=text_pad_lengths,
                input_tokens=None,
                do_sample=do_sample,
                top_p=top_p,
                top_k=top_k,
            ...
            for i, gpt_codes in enumerate(batch_gpt_codes):

                text_tokens = text_ids[i].unsqueeze(0).to(self.device)

                #--------------------------------
                # Remove the padding tokens
                audio_pad_id = 1025
                gpt_codes = gpt_codes.tolist()
                last_pad_idx = 0
                for code in gpt_codes[::-1]:
                    if code == audio_pad_id:
                        last_pad_idx += 1
                    else:
                        break
                gpt_codes = gpt_codes[:len(gpt_codes) - last_pad_idx]
                gpt_codes = torch.IntTensor(gpt_codes).unsqueeze(0).to(self.device)
                #--------------------------------
